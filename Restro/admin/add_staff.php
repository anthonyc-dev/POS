...
if (isset($_POST['addStaff'])) {
  ...
  if (empty($_POST["staff_number"]) || empty($_POST["staff_name"]) || empty($_POST['staff_email']) || empty($_POST['staff_password'])) {
      ...
      // Validate password (at least one lowercase, one uppercase, and one special character)
      if (!preg_match('/^(?=.*[a-z])(?=.*[A-Z])(?=.*[\W_]).+$/', $staff_password)) {
          $err = "Password must include at least one lowercase letter, one uppercase letter, and one special character.";
      } else {
          // Check if email already exists
          $checkEmailQuery = "SELECT staff_email FROM rpos_staff WHERE staff_email = ?";
          ...
          if ($checkEmailStmt->num_rows > 0) {
              ...
              if ($postStmt) {
                  $success = "Staff Added Successfully" && header("refresh:1; url=hrm.php");
              } else {
