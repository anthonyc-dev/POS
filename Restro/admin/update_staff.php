...
if (isset($_POST['UpdateStaff'])) {
  ...
  if (empty($_POST["staff_number"]) || empty($_POST["staff_name"]) || empty($_POST['staff_email']) || empty($_POST['staff_password'])) {
      ...
      $staff_password = $_POST['staff_password']; // Store password as plain text
      $update = $_GET['update'];

      // Check if email already exists (excluding current staff's email)
      $checkEmailQuery = "SELECT staff_email FROM rpos_staff WHERE staff_email = ? AND staff_id != ?";
      $checkEmailStmt = $mysqli->prepare($checkEmailQuery);
      $checkEmailStmt->bind_param('si', $staff_email, $update);
